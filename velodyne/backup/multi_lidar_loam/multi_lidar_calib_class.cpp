...
bool msy_multi_lidar::process()
{
  ...
  pcl::toROSMsg(CloudTL, msy_laserCloudqqqq);
  msy_laserCloudqqqq.header.stamp = _timeTL;
  ...
  msy_laserCloudqqqq.header.frame_id = "velodyne";
  pub5.publish(msy_laserCloudqqqq);


  pcl::PointCloud<pcl::PointXYZI>::Ptr calib(new pcl::PointCloud<pcl::PointXYZI>());

  size_t cloudSize1 = CloudTL.points.size();
  size_t cloudSize2 = CloudTC.points.size();
  size_t cloudSize3 = CloudTR.points.size();

  for (size_t i = 0; i < cloudSize1; i++) {
    calib->push_back(CloudTL[i]);
  }

  for (size_t i = 0; i < cloudSize2; i++) {
    calib->push_back(CloudTC[i]);
  }

  for (size_t i = 0; i < cloudSize3; i++) {
    calib->push_back(CloudTR[i]);
  }

  pcl::toROSMsg(*calib, msy_laserCloudqqqq);
  msy_laserCloudqqqq.header.stamp = _timeTC;
  msy_laserCloudqqqq.header.frame_id = "velodyne";
  pub_calib.publish(msy_laserCloudqqqq);
